<template>
    <div class="table">
        ...
        <el-table :data="tableData" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="prizeType" label="奖品名称" width="120">
            </el-table-column>
            <el-table-column prop="redeemCode" label="兑换码">
            </el-table-column>
			<el-table-column prop="exchange" label="兑换状态" width="120" :formatter="isExchangeFormatter">
            </el-table-column>
            <el-table-column prop="time" v-text="" label="兑换时间" sortable width="180" :formatter="timeShowFormat">
            </el-table-column>

            <el-table-column label="操作" width="80">
                <template scope="scope">
                    <el-button size="small" :infocontent="infocontent" type="primary"
                            @click="handleEdit(scope.$index, scope.row)" v-show="scope.row.exchange!=1">兑换</el-button>            
                </template>
            </el-table-column>
        </el-table>
...
//              			
        ...
        methods: {
            ...
            getData(){
                let self = this;
				ajaxRequest({
					url : "/hemsCircleQueryAllPrizess",
					data : {
						"endTime":"",
						"issued":"",
						"pageNum":self.cur_page.toString(),
						"pageSize":"10",
						"prizeType":"",
						"redeemCode":"",
						"startTime":""
					...
					success : function(dataJson) {
						self.total = parseInt(dataJson.result.total);
						self.tableData = dataJson.result.root;
					},
					error : function() {
						alert("系统繁忙");
					}
            ...
            timeShowFormat(row, column){
            	if (row.time != "") {
            		return  row.time.substring(0,4)+"-"+
					    row.time.substring(4,6)+"-"+
						row.time.substring(6,8)+" "+
						row.time.substring(8,10)+":"+
						row.time.substring(10,12);
            	} else {
            		return "";
            	}
            ...
            formatter(row, column) {
                return row.redeemCode;
            },
            isExchangeFormatter(row, column) {
                return row.exchange == "1" ? "已兑换":"未兑换";
            },
            filterTag(value, row) {
