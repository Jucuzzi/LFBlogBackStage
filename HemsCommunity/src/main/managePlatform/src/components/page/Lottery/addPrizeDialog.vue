	...
	methods: {
		...
		confirm(){
			...
			if (!(/(^[1-9]\d*$)/.test(this.input_prize)) || this.input_prize >
				...
				this.selectPrizeInfo.remainNum = (parseInt(this.selectPrizeInfo.remainNum) + parseInt(this.input_prize)).toString();
				this.selectPrizeInfo.sumNum = (parseInt(this.selectPrizeInfo.sumNum) + parseInt(this.input_prize)).toString();
				  let self = this;
				ajaxRequest({
					url : "/hemsCircleAddNewPrizes",
					data : {
						"prizeNum":self.input_prize,
						"prizeType":self.selectPrizeInfo.prizeType
					},
					async : true,
