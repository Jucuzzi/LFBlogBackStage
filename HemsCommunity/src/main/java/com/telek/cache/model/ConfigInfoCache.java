...
public class ConfigInfoCache extends ObserverModel {
	 ...
	 * 举报类型
	 */
	public static Map<String, String> reportType;
	/**
	 * 举报内容类型
	 */
	public static Map<String, String> reportContentType;
	...
	public void loadConfigInfoCache() {
		...
		if (configMap.containsKey("reportType")) {
			String[] type = configMap.get("reportType").split(",");
			for (int i = 0; i < type.length; i++){
				if (configMap.containsKey(type[i])) {
					reportType.put(type[i], configMap.get(type[i]));
				}
