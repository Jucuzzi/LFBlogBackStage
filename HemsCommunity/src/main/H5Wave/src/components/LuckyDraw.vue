<template>
	<div class="hello" id="backgroundDiv">
		<div class="sign-titleBar">
			...
			<div class="rightDiv"  v-on:click="showHistory">
							...
							<ul class="ul-style">
								<li v-for="item of ticketList" class="record-background">
									...
									<div class="record-child-item-bottom">
										<span class="record-child-item-bottom-left" v-text="showPrizeName(item.prizeType)">
									...
									<img v-if="item.exchange==1" class="overlap-icon" src="../telehems/img/overlap_icon.png" />
								</li>
							</ul>
						</div>
					</div>
	...
	export default {
		...
		methods: {
			...
			showHistory: function() {
				var self = this;
				ajaxRequest({
					url : "/hemsCircleQueryPrizeDrawHistory",
					data : {
					...
					success : function(dataJson) {
						if (dataJson.result.result == "failed") {
							$('.record-body').html("暂无中奖信息");
						} else {
							for (var index=0;index<dataJson.result.root.length;index++){
							var showTime = dataJson.result.root[index].time;
							 dataJson.result.root[index].time = showTime.substring(0,4)+"-"+
					    			showTime.substring(4,6)+"-"+
								showTime.substring(6,8)+" "+
								showTime.substring(8,10)+":"+
								showTime.substring(10,12);
							}
						
							self.ticketList=dataJson.result.root;

							if (self.ticketList.length < 1){
							//没有中奖信息时候
								$('.record-body').html("暂无中奖信息");
							} else {
