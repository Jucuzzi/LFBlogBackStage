<template>
	<div>
		...
		<ul class="dynamic-ul-style" v-if="!netError">
			<li class="dynamic-li-tittle">
				<span class="dynamic-tittlr-text">今日</span>
			</li>
			<div v-if="todayStealList.length==0" style="border-bottom: 1px solid #dedede;">
				<img src="../assets/img/noStealRecord.png"></img>
				<br/>
				<label style="color: lightgray; font-weight: 300; margin-bottom: 10px;">还没有人来偷过你的能量哦</label>
			</div>
			<li class="dynamic-sign-list-root" v-for="item in todayStealList"  v-if="todayStealList.length!=0">
				<img class="dynamic-user-icon" :src="item.userIconPath"/>
			...
			<div v-if="yesterdayStealList.length==0" style="border-bottom: 1px solid #dedede;">
				<img src="../assets/img/noStealRecord.png"></img>
				<br/>
				<label style="color: lightgray; font-weight: 300; margin-bottom: 10px;">还没有人来偷过你的能量哦</label>
			</div>
			<li class="dynamic-sign-list-root" v-for="item in yesterdayStealList"  v-if="yesterdayStealList.length!=0">
				<img class="dynamic-user-icon" :src="item.userIconPath"/>
				<div class="dynamic-sign-list-item-center">
					<span class="dynamic-user-name">{{item.userName}}</span>
					<span class="dynamic-get-num">收取 {{item.stealEnergy}}g</span>
		...
		<!-- 网络异常的显示 -->
		<div class="list-frands" v-if="netError" style="text-align: center; margin-top: 60px;">
			<div>
			...
			getData() {
				...
				ajaxRequest({
					...
					success: function(dataJson) {
						...
						self.yesterdayStealList = dataJson.result.root[1].stealUser;
					},
					error: function() {
						self.netError = true;
